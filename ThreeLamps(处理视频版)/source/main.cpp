...
void main(void)
{
	...
	unsigned char sample_suc = 0;  //采集图片成功标志
	/******************************连接相机，设置参数*******************************/
	int nRet = -1;
	void* m_handle = NULL;
	...
	if (MV_OK != nRet)
	{
		cout<<"error: SetFloatValue fail \n";
		system("pause");
		return;
	}

	/*float frate = 17;
	nRet = MV_CC_SetFloatValue(m_handle, "ResultingFrameRate", frate);
	if (MV_OK != nRet)
	{
		cout<<"error: SetFloatValue fail \n";
		system("pause");
	...
	if (nRet != MV_OK)
	{
		cout<<"Get PayloadSize failed! nRet \n";
		system("pause");
		return;
	}
	...
	gray_image = Mat::zeros(height,width, CV_8UC1);
	//每17张图片处理一次
	cout<<"相机成功打开"<<endl;
	while(1)
	{
		for (num = 1; num <= 1; num++)
		{
			...
			else
			{
				...
				for (i = 0; i < height; i++)
				{
					for (j = 0; j < width; j++)
					{
						gray[num-1][i][j] = pFrameBuf[i * width + j];
						gray_image.data[i * width + j] = pFrameBuf[i * width + j];
					}
				}
			}
			...
			/*VideoCapture cap("C:\\Users\\67081\\Documents\\WeChat Files\\wxid_znut3kdb81hz22\\FileStorage\\File\\2021-08\\定位算法测试素材811\\定位算法测试素材811\\Video_1.avi");
			if(!cap.isOpened())
			{
			...
			if(sample_suc==1)
			{
				...
				if(threelight.light_num>=0)
				{
					...
					int** source1 = (int**)malloc(sizeof(int*) * (W[0] + 500));
					for (i = 0; i < W[0]; i++)
					{
						source1[i] = (int*)malloc(sizeof(int) * (H[0] + 500));
					}
					int** source2 = (int**)malloc(sizeof(int*) * (W[1] + 500));
					for (i = 0; i < W[1]; i++)
					{
						source2[i] = (int*)malloc(sizeof(int) * (H[1] + 500));
					}
					int** source3 = (int**)malloc(sizeof(int*) * (W[2] + 500));
					for (i = 0; i < W[2]; i++)
					{
						source3[i] = (int*)malloc(sizeof(int) * (H[2] + 500));
					}
					int** source4 = (int**)malloc(sizeof(int*) * (W[3] + 500));
					for (i = 0; i < W[3]; i++)
					{
						source4[i] = (int*)malloc(sizeof(int) * (H[3] + 500));
					}
					...
					if(threelight.light_num>=5)
					{
						...
						int** source5 = (int**)malloc(sizeof(int*) * (W[4] + 500));
						for (i = 0; i < W[4]; i++)
						{
							source5[i] = (int*)malloc(sizeof(int) * (H[4] + 500));
						}
						...
						for (i = 0; i < W[4]; i++)
						{
							for (j = 0; j < H[4]; j++)
							{
								source5[i][j] = gray[num-1][threelight.light_range[4][0] + j][threelight.light_range[4][3] - 1 - i];
					...
					for (i = 0; i < W[0]; i++)
					{
						for (j = 0; j < H[0]; j++)
						{
							source1[i][j] = gray[num-1][threelight.light_range[0][0] + j][threelight.light_range[0][3] - 1 - i];
					...
					for (i = 0; i < W[1]; i++)
					{
						for (j = 0; j < H[1]; j++)
						{
							source2[i][j] = gray[num-1][threelight.light_range[1][0] + j][threelight.light_range[1][3] - 1 - i];
					...
					for (i = 0; i < W[2]; i++)
					{
						for (j = 0; j < H[2]; j++)
						{
							source3[i][j] = gray[num-1][threelight.light_range[2][0] + j][threelight.light_range[2][3] - 1 - i];
					...
					for (i = 0; i < W[3]; i++)
					{
						for (j = 0; j < H[3]; j++)
						{
							source4[i][j] = gray[num-1][threelight.light_range[3][0] + j][threelight.light_range[3][3] - 1 - i];
